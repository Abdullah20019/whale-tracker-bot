...
bot_state = {
    ...
    'multi_buys': {},
    'whale_performance': {},
    'whale_token_balances': {}  # NEW: Track whale balances for sell detection
}
...
def check_whale_sells_solana():
    ...
    for whale_key, balance_data in list(bot_state['whale_token_balances'].items()):
        ...
        # Only check if last check was >2 min ago
        ...
        # Detect sell (balance decreased by 30%+)
        if initial_balance > 0:
            balance_change_pct = ((current_balance - initial_balance) / initial_balance) * 100
            
            if balance_change_pct < -30:  # Sold 30%+ of position
                # Send sell alert
...
"""
                    ...
                    # Mark as sold in tracked tokens
                    if token_address in bot_state['tracked_tokens']:
...
while True:
    ...
    for i, whale in enumerate(solana_whales, 1):
        ...
        current_tokens_data = get_solana_tokens(address)
        current_tokens = {t['address'] for t in current_tokens_data}
        previous_tokens = whale_tokens.get(address, set())
        new_token_addresses = current_tokens - previous_tokens
        
        if new_token_addresses and not first_run:
            for token_address in new_token_addresses:
                token_balance = next((t['balance'] for t in current_tokens_data if t['address'] == token_address), 0)
                token_info = get_token_info(token_address, 'solana')
                ...
                if not passed:
                    ...
                    print(f"  ⏭️  Filtered: {token_info['symbol'] if token_info else 'UNK'} - {reason}")
                    continue
                
                new_buys_found += 1
                buy_amount_usd = token_balance * token_info['price']
                
                print(f"\n🚨 NEW BUY!")
                print(f"   {token_info['symbol']} | MC: ${token_info['market_cap']:,.0f}")
                
                # Start tracking with balance
                track_token_buy(
                    token_address,
                    address,
                    token_info['price'],
                ...
                whale_count = check_multi_buy(token_address)
                multi_buy_alert = ""
                ...
                confidence = "🟢 HIGH" if whale_count >= 3 else "🟡 MEDIUM" if whale_count >= 2 else "⚪ STANDARD"
...
💎 <b>{token_info['symbol']}</b> | {token_info['name']}
{multi_buy_alert}
...
📈 Volume 24h: <b>${token_info['volume_24h']:,.0f}</b>

📈 Price Changes:
   5m: <b>{token_info['price_change_5m']:.2f}%</b>
   1h: <b>{token_info['price_change_1h']:.2f}%</b>

🔄 24h Activity:
   Buyers: <b>{buyers}</b> | Sellers: <b>{sellers}</b>

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🐋 <b>WHALE INFO</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📍 Address: <code>{address[:16]}...</code>
💵 Buy Amount: <b>${buy_amount_usd:,.2f}</b>
🪙 Tokens: <b>{token_balance:,.0f}</b>
🏆 Win Rate: <b>{whale.get('win_rate', 0):.1f}%</b>
🎯 Whales Bought: <b>{whale_count if whale_count > 0 else 1}</b>
...
🔔 Monitoring: Price updates + Whale exits
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
                ...
                if send_telegram_alert(message):
                    ...
                    bot_state['last_buys'] = bot_state['last_buys'][-20:]
                    save_bot_state()
        
        whale_tokens[address] = current_tokens
    ...
    for i, whale in enumerate(base_whales, 1):
        address = whale['address']
        current_tokens_data = get_base_tokens(address)
        current_tokens = {t['address'] for t in current_tokens_data}
        previous_tokens = whale_tokens.get(address, set())
        new_token_addresses = current_tokens - previous_tokens
        ...
        if new_token_addresses and not first_run:
            for token_address in new_token_addresses:
                ...
                new_buys_found += 1
                token_balance_readable = token_balance / (10 ** 18)
                buy_amount_usd = token_balance_readable * token_info['price']
...
📍 Address: <code>{address[:16]}...</code>
💵 Buy Amount: <b>${buy_amount_usd:,.2f}</b>
🪙 Tokens: <b>{token_balance_readable:,.0f}</b>
🏆 Win Rate: <b>{whale.get('win_rate', 0):.1f}%</b>
🎯 Whales Bought: <b>{whale_count if whale_count > 0 else 1}</b>
...
"""
    ...
    if first_run:
        ...
        print(f"✅ Cycle: {new_buys_found} alerts | {filtered_out} filtered")
