...
try:
    ...
    # Try to import TELEGRAM_GROUP_ID, but don't fail if it doesn't exist
    try:
...
def send_telegram_message(message, chat_id=None, parse_mode=None):
    ...
    If chat_id not specified, sends to private chat (and group if configured)
    """
    ...
    if chat_id:
        ...
        # Send to private chat (always) and group chat (if configured)
        target_chats = []
        if TELEGRAM_CHAT_ID:
...
def get_wallet_transactions(wallet_address, chain='sol'):
    ...
    Placeholder - implement with Helius (Solana) or Alchemy (Base)
    """
    return []
